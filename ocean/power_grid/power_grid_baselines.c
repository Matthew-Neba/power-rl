...
static int search_at_depth(const PowerGridTopology* topology, const PowerGridOperatingPoint* point,
        int remaining, int previous_action, int* path, int path_length, PowerGridSearchResult* result) {
    ...
    for (int action = 1; action < POWER_GRID_NUM_ACTIONS; action++) {
        ...
        if (search_at_depth(&candidate, point, remaining - 1, action, path, path_length + 1, result)) {
            return 1;
        }
    }
    return 0;
...
PowerGridSearchResult power_grid_search_safe_topology(const PowerGridTopology* topology,
        const PowerGridOperatingPoint* point, int max_depth) {
    PowerGridSearchResult result = {0};
    int path[POWER_GRID_MAX_SEARCH_DEPTH] = {0};
    if (topology == NULL || point == NULL) return result;
    if (max_depth < 0) max_depth = 0;
    if (max_depth > POWER_GRID_MAX_SEARCH_DEPTH) max_depth = POWER_GRID_MAX_SEARCH_DEPTH;
    for (int depth = 0; depth <= max_depth; depth++) {
        if (search_at_depth(topology, point, depth, -1, path, 0, &result)) return result;
    }
    return result;
}
